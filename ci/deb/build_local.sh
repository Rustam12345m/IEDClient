#!/bin/bash
# Build IEDClient and package it as a .deb inside a Docker/Podman container.
#
# Usage:
#   ./ci/deb/build_local.sh                  # Build + create .deb
#   ./ci/deb/build_local.sh --no-build       # Re-package only (reuse install/)
#   ./ci/deb/build_local.sh --rebuild-image  # Force rebuild the container image
#
# Environment:
#   DOCKER=podman ./ci/deb/build_local.sh    # Use podman instead of docker
...
Builds IEDClient inside an Ubuntu container and produces a .deb package.
...
Artifact is stored in:
  dist/             The generated .deb package
EOF
    exit 0
}
...
build_image() {
    ...
    if [ -z "$image_exists" ] || $REBUILD_IMAGE; then
        ...
        "$DOCKER" build -f "$SCRIPT_DIR/../Dockerfile.Ubuntu" -t "$IMAGE_NAME" "$SCRIPT_DIR/../"
    else
...
run_package() {
    ...
    "$DOCKER" run "${run_opts[@]}" \
        "$IMAGE_NAME" \
        "$CONTAINER_WORKDIR/ci/deb/create_package.sh" "${PACKAGE_ARGS[@]}"
}
...
if [ -d "$REPO_DIR/dist" ]; then
    ...
    ls -lh "$REPO_DIR"/dist/*.deb 2>/dev/null || ls -lh "$REPO_DIR/dist/"
fi
echo "==> Done."
