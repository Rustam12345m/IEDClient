...
#include "rcb_main_tablemodel.hpp"

RCB_MainTableModel::RCB_MainTableModel(QObject *t_parent, QSharedPointer<Core::IED_Object> &t_ied)
	: QAbstractTableModel(t_parent), m_ied(t_ied)
{
}

void RCB_MainTableModel::setNewIED(QSharedPointer<Core::IED_Object> t_ied)
{
	beginResetModel();
	m_ied = t_ied;
	endResetModel();
...
QVariant RCB_MainTableModel::headerData(int t_column, Qt::Orientation t_orientation, int t_role) const
{
...
QHash<int, QByteArray> RCB_MainTableModel::roleNames() const
{
	return { { Qt::DisplayRole, "display" } };
}

int RCB_MainTableModel::rowCount(const QModelIndex &t_parent) const
{
	/*
...
int RCB_MainTableModel::columnCount(const QModelIndex &t_parent) const
{
	return ColumnsCount;
}

QVariant RCB_MainTableModel::data(const QModelIndex &t_index, int t_role) const
{
	//qDebug() << "RCB_MainTableModel: " << QString("index = %1 %2, role = %3").arg(t_index.row()).arg(t_index.column()).arg(t_role);
...
void RCB_MainTableModel::slotDataUpdated(bool t_done)
{
}